...
const Home = () => {
    ...
    return (
        <>
                    ...
                    (
                        categories && categories.length > 0 ? (
                            <div className='my-2 mx-auto w-fit max-w-full md:my-4 py-2 px-4 flex items-center gap-2 overflow-x-hidden hover:overflow-x-auto'>
                                <div className={`border-[0.5px] rounded-full px-4 py-2 cursor-pointer hover:bg-gray-100 ${selectedCategory === 'All' && 'bg-gray-100'}`}
                                ...
                                </div>
                                {
                                    categories.map((category) => (
                                        <div key={category._id}
                                            className={`w-fit text-nowrap border-[0.5px] rounded-full px-4 py-2 cursor-pointer hover:bg-gray-100 ${selectedCategory === category._id && 'bg-gray-100'}`}
            ...
            <Link to='/note'>
                <div className='fixed bottom-14 right-6 bg-yellow-200 p-3 rounded-full cursor-pointer shadow-md hover:bg-yellow-300 z-10'>
                    <Add fontSize='large' />
                </div>
            </Link>
        </>
